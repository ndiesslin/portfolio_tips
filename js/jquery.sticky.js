    ...
    this.each(function () {
      ...
      var sticky = {
        ...
        'init': function ($this) {
          ...
          sticky.units = { 'start': sticky.element.offset().top };
          settings.states = [
          ...
          // Find Stopper
          if (settings.stopper != '') {
            var stoppers = $(settings.stopper), margin;
        ...
        'update' : function () {
          ...
          if((sticky.element.outerHeight(false) + settings.offset) < $(window).height()) {
            ...
            if (
              ...
              && (sticky.units.doctop + settings.offset + sticky.element.outerHeight(false)) >= sticky.units.stop
            ) {
              // Stop
            ...
            // Update Position
              ...
              && (typeof(sticky.stopper) == 'undefined' || sticky.stopper.length == 0 || (sticky.stopper.length > 0 && (sticky.units.doctop + settings.offset + sticky.element.outerHeight(false)) < sticky.units.stop))
            ) {
              // Stick or Animate
        ...
        'stop': function (top,location) {
          sticky.element
            .removeClass(settings.states)
            .addClass(location == 'start' ? settings.classes.start : settings.classes.stopped);
